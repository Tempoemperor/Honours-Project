...
class Blockchain:
    ...
    def _verify_transaction(self, transaction: Transaction) -> bool:
        ...
        if transaction.signature:
            ...
            if derived_address != transaction.sender:
                print(
                    f"[Validation Fail] Public key derives to {derived_address[:8]}, "
                    f"but sender is {transaction.sender[:8]}"
                )
                return False
    ...
    def _execute_permission_grant(self, tx: Transaction) -> None:
        ...
        if self.permission_system and 'new_level' in tx.data:
            ...
            if success:
                ...
                print(
                    f"Promotion failed: {tx.sender[:8]}... "
                    f"cannot promote {target[:8]}... to {new_level}"
                )
    ...
    def _execute_custom_transaction(self, tx: Transaction) -> None:
        """
        Handle custom transaction types that are not core monetary / validator ops.
        Currently used for FILE_STORE audit entries.
        """
        if tx.tx_type == TransactionType.FILE_STORE:
            if not self.permission_system:
                return

            data_id        = tx.data.get("data_id")
            security_level = tx.data.get("security_level")
            name           = tx.data.get("name")
            owner          = tx.data.get("owner") or tx.sender

            # Do not touch actual data_store here; content is already written by store_data().
            self.permission_system._log_action(
                "file_store_tx",
                tx.sender,
                {
                    "data_id":        data_id,
                    ...
                    "owner":          owner,
                    "tx_hash":        tx.hash(),
                    "block_height":   self.get_height() + 1,
                },
            )
            return

        # Future custom transaction types can be handled here.
        return
    ...
    def store_data(
        ...
        return self.permission_system.store_data(
            data_id, content, security_level, owner_address, metadata
        )
